...
int main(int argc, char* argv[])
{
	...
	for(unsigned int i = 0 ; i < v.size () ; i++)
		...
		{
			//fprintf(fdef,"%s=%s @%u\r\n",v[i].en,v[i].in,v[i].o); //yoel
			fprintf(fdef,"%s=_%s @%u\r\n",v[i].en,v[i].en,v[i].o+42);
		}
		else
	...
	fprintf(fcpp,"//////////// \r\n");
	fprintf(fcpp,"//////////// replace interesting functions manually with the actual implementation (correct func signature etc.)\r\n");
	fprintf(fcpp,"//////////// \r\n");
	...
	for(unsigned int i = 0 ; i < v.size() ; i++)
		{
		//fprintf(fcpp,"// %s\r\nextern \"C\" __declspec(naked) void %s %s()\r\n",v[i].en,argv[3],v[i].in);//yoel		
		fprintf(fcpp,"// %s\r\nextern \"C\" __declspec(naked) void %s _%s()\r\n",v[i].en,argv[3],v[i].en);
		fprintf(fcpp,"\t{\r\n");
		fprintf(fcpp,"\t__asm\r\n");
		fprintf(fcpp,"\t\t{\r\n");
